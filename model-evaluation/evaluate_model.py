...
if __name__ == '__main__':
    ...
    with open(os.path.join(WORKDIR, '../data-processing/data/test', test_data + '.csv'), 'r') as f:
        test = pd.read_csv(f)
...
#   with open(os.path.join(WORKDIR, model_name + '.pkl'), 'rb') as f:
    with open(os.path.join(WORKDIR, '../data-modeling/Linear_models', model_name + '.pkl'), 'rb') as f:
        model = pickle.load(f)
...
#        run.log(metric, value)
    ...
    plot_metric('MAPE', performance_metrics, model_name)
    plot_metric('MdRAE', performance_metrics, model_name)
    plot_metric('MPE', performance_metrics, model_name)

    # Output the predictions dataframe
    with open(os.path.join('.', 'outputs', model_name + '_predictions.pkl'), 'wb') as f:
